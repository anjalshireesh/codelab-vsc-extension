...
import {
  COMMAND_COPY_CLIPBOARD,
  COMMAND_EXECUTE,
  COMMAND_EXECUTE_INTERRUPT,
  COMMAND_OPEN,
  COMMAND_PREVIEW,
} from "./consts";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(
      (_) => (config = CodeLabConfig.getCurrentConfig())
    )
  );
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((event) =>
      changeTextSubject.next(event)
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(COMMAND_COPY_CLIPBOARD, onCopy)
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(COMMAND_EXECUTE, onExecute)
  );
  context.subscriptions.push(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(COMMAND_OPEN, onOpen)
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(COMMAND_PREVIEW, (_) => onPreview(context))
  );
  ...
  subscription.add(
    changeTextSubject
      ...
      .subscribe((event) => onDidChangeTextDocument(event, context))
  );
    ...
    if (path.isAbsolute(filePath)) {
      ...
      switch (config.openRelativeConfig) {
        case OpenRelativeConfig.File:
          const fsPath =
            CodeLabEditorProvider.currentFile ||
            vscode.window.activeTextEditor?.document.uri.fsPath ||
            CodeLabWebViewPanel.currentPanel?.sourceDocument?.uri.fsPath;
          if (fsPath) {
